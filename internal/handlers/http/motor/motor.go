...
type MotorHandler struct {
	Service interfacemotor.ServiceMotorInterface
}

func NewMotorHandler(service interfacemotor.ServiceMotorInterface) *MotorHandler {
	return &MotorHandler{Service: service}
}

func (h *MotorHandler) GetAll(ctx *gin.Context) {
...
func (h *MotorHandler) GetByID(ctx *gin.Context) {
	logId := utils.GenerateLogId(ctx)
	id := ctx.Param("id")
	if id == "" {
		res := response.Response(http.StatusBadRequest, messages.InvalidRequest, logId, nil)
...
func (h *MotorHandler) Create(ctx *gin.Context) {
	logId := utils.GenerateLogId(ctx)
	var req dto.MotorTypeRequest
...
func (h *MotorHandler) Update(ctx *gin.Context) {
	logId := utils.GenerateLogId(ctx)
...
func (h *MotorHandler) Delete(ctx *gin.Context) {
	logId := utils.GenerateLogId(ctx)
	id := ctx.Param("id")
	if id == "" {
		res := response.Response(http.StatusBadRequest, messages.InvalidRequest, logId, nil)
